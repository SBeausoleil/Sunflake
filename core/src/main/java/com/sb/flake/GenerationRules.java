...
public class GenerationRules implements Serializable {
    ...
    public FlakeData parse(long flake) {
        long msSinceEpoch = flake >> this.TIMESTAMP_SHIFT & this.TIMESTAMP_MASK;
        msSinceEpoch *= this.TIME_UNITS_PER_TICK; // Decompress if there were multiple units per tick
        msSinceEpoch = this.TIME_UNIT.convert(msSinceEpoch, TimeUnit.MILLISECONDS);
        Instant timestamp = this.EPOCH.plusMillis(msSinceEpoch);
        long workerId = flake >> this.getWorkerIdShift() & this.WORKER_ID_MASK;
        long sequenceNumber = flake & this.SEQUENCE_MASK;
